...
export class SFTPFileSystem implements vscode.FileSystemProvider {
    ...
    cachedRemoteDirectoryFiles = new Map<string, DirectoryCachedFiles>();
    ...
    private setupFileSystem(uri: vscode.Uri) {
        ...
        const newData = new SFTPFileProviderData(this.extension);
        newData.setupDone = true;
        newData.remoteName = uri.authority;
        newData.remoteConfiguration = this.extension.configuration.getRemoteConfiguration(newData.remoteName) ?? {};
        const current = (this.extension.configuration.getWorkDirForRemote(newData.remoteName));
    ...
    $stat(uri: vscode.Uri, fallbackToLocalFile: boolean, connectionType: PoolType): Promise<vscode.FileStat> | vscode.FileStat {
        ...
        if (uri.path.startsWith('/.vscode')) {
            this.extension.logger.appendLineToMessages('[stat] Skipped stat: ' + uri.path);
        ...
        this.extension.logger.appendLineToMessages('[stat] ' + uri.path);
        ...
        if (cachedDirectory !== undefined) {
            this.extension.logger.appendLineToMessages('[stat] Cached directory found for parent of ' + uri.path + ': ' + parentDirUri.path);
            ...
            if (fallbackToLocalFile) {
                ...
                return new Promise(async (resolve, reject) => {
                    ...
                    if (localStats === undefined) {
                        ...
                        this.extension.logger.appendLineToMessages('File not found when stat file (' + remoteName + '): ' + uri.path);
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                if (connection === undefined) {
                    this.extension.logger.appendLineToMessages('Error when stat file (' + remoteName + '): Connection lost.');
                    vscode.window.showErrorMessage('Broken connection to SFTP server.');
                    return;
                }
                
                try {
                    this.extension.logger.appendLineToMessages('[stat] Not cached: ' + uri.path);
                    
                    // Read content of parent folder an cache it
                    this.extension.logger.appendLineToMessages('[stat] Trying to read directory for cache: ' + parentDirUri.path);
                    ...
                    if (this.shouldCacheDirectory(parentDirUri)) {
                            ...
                            connection.stat(parentDirUri.path, (err: any) => {
                                if (err) {
                                    if (err.code === ErrorCodes.FILE_NOT_FOUND) {
                                        this.extension.logger.appendErrorToMessages('$stat', 'File not found when stat directory for cache (' + remoteName + '): ' + uri.path, err);
                                        reject(vscode.FileSystemError.FileNotFound(uri));
                                    } else {
                                        reject(err);
                                    }
                        ...
                        this.extension.logger.appendLineToMessages('[stat] [cache] Cache is not required for: ' + parentDirUri.path);
                    }
                    ...
                    try {
                        ...
                        await this.releaseConnection(remoteName, connectionProvider);
                        this.extension.logger.appendErrorToMessages('$stat', 'Error on (' + remoteName + '): ' + uri.path, ex);
                        reject(ex);
                    }
                } catch(error: any) {
                    await this.releaseConnection(remoteName, connectionProvider);
                    if (error.code === ErrorCodes.FILE_NOT_FOUND || error instanceof vscode.FileSystemError.FileNotFound) {
                        this.extension.logger.appendErrorToMessages('$stat', 'File not found when stat file (' + remoteName + '): ' + uri.path, error);
                        reject(vscode.FileSystemError.FileNotFound(uri));
                    } else {
                        console.log(error);
                        this.extension.logger.appendErrorToMessages('$stat', 'Error when stat file (' + remoteName + '): ' + uri.path, error);
    ...
    async $statPerform(uri: vscode.Uri, connection: SFTPWrapper, fileStats: Stats | vscode.FileStat, isLocalOnly: boolean) {
        ...
        this.extension.logger.appendLineToMessages('[stat] $statPerform for ' + uri.path);
                    ...
                    try {
                        ...
                        if (localFileStat === undefined) {
                            if (fileType !== vscode.FileType.Directory) {
                                this.extension.sftpFileDecoration.setRemoteFileDecoration(uri);
                            }
                        } else {
                            if (localFileStat.type !== vscode.FileType.Directory) {
                                const res = await this.resolveWhatFileIsNewer(localFileStat, fileStats);
                                if (res === "same") {
                                    this.extension.sftpFileDecoration.setUpToDateFileDecoration(uri);
                                } else if(res === 'local_newer') {
                                    this.extension.sftpFileDecoration.setLocalUploadFileDecoration(uri);
                                } else if(res === 'remote_newer') {
                                    this.extension.sftpFileDecoration.setRemoteDownloadFileDecoration(uri);
                                } else {
                                    this.extension.sftpFileDecoration.setUnknownStateFileDecoration(uri);
                                }
                            }
                        }
                    } finally {
    ...
    async readDirectory(uri: vscode.Uri): Promise<[string, vscode.FileType][]> {
        return this.$readDirectory(uri, true);
    }
    
    async $readDirectory(uri: vscode.Uri, useCache: boolean): Promise<[string, vscode.FileType][]> {
        const remoteName = this.getRemoteName(uri);
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                this.extension.logger.appendLineToMessages('[read-dir] ' + uri.path);
                
                // Resolve local files...
                const result: [string, vscode.FileType][] = [];
                const listLocalFunc = async (fileNames: string[] | undefined) => {
                    const localUri = this.getLocalFileUri(remoteName, uri);
                    const localStats = await this.statLocalFileByUri(localUri);
                        ...
                        localFiles.forEach((local) => {
                            ...
                            if (fileNames !== undefined) {
                                for (const fileName of fileNames) {
                                    if (local[0].toLowerCase() === fileName.toLowerCase()) {
                                        found = true;
                                        break;
                                    }
                                }
                            ...
                            if (!found) {
                                ...
                                this.extension.sftpFileDecoration.setLocalUploadFileDecoration(remoteLocalUri);
                            }
                        });
                    }
                };
                ...
                if (useCache) {
                    const cacheEntry = this.cachedRemoteDirectoryFiles.get(uri.toString());
                    if (cacheEntry !== undefined) {
                        this.extension.logger.appendLineToMessages('[read-dir] Using cached remote files for ' + uri.path);
                        for (const entry of cacheEntry.files) {
                            result.push([entry[0], entry[1]]);
                        }
                        
                        await listLocalFunc(cacheEntry.files.map((f) => f[0]));
                        resolve(result);
                        return;
                    }
                }
                ...
                connection.readdir(uri.path, async (err: any, stats) => {
                    ...
                    try {
                        ...
                        this.cachedRemoteDirectoryFiles.set(
                            uri.toString(), 
                            {
                                files: stats.map((s) => [s.filename, this.getFileTypeByStats(s.attrs)])
                            }
                        );
                        ...
                        for (const entry of stats) {
                            ...
                            if (localFileStat === undefined) {
                                if (fileType !== vscode.FileType.Directory) {
                                    this.extension.sftpFileDecoration.setRemoteFileDecoration(uri.with({ path: upath.join(uri.path, entry.filename) }));
                                }
                            } else {
                                if (localFileStat.type !== vscode.FileType.Directory) {
                                    const res = await this.resolveWhatFileIsNewer(localFileStat, entryStats);
                                    if (res === "same") {
                                        this.extension.sftpFileDecoration.setUpToDateFileDecoration(uri.with({ path: upath.join(uri.path, entry.filename) }));
                                    } else if(res === 'local_newer') {
                                        this.extension.sftpFileDecoration.setLocalUploadFileDecoration(uri.with({ path: upath.join(uri.path, entry.filename) }));
                                    } else if(res === 'remote_newer') {
                                        this.extension.sftpFileDecoration.setRemoteDownloadFileDecoration(uri.with({ path: upath.join(uri.path, entry.filename) }));
                                    } else {
                                        this.extension.sftpFileDecoration.setUnknownStateFileDecoration(uri.with({ path: upath.join(uri.path, entry.filename) }));
                                    }
                                }
                            }
                        }
                        
                        await listLocalFunc(stats.map((f) => f.filename));
                        await this.releaseConnection(remoteName, connectionProvider);
                        resolve(result);
                    } catch(ex: any) {
                        await this.releaseConnection(remoteName, connectionProvider);
                ...
                this.extension.logger.appendLineToMessages('Cannot read directory (' + remoteName + '): ' + ex.message);
    ...
    async readFile(uri: vscode.Uri): Promise<Uint8Array> {
        ...
        this.extension.logger.appendLineToMessages('[read-file] Requested readfile from vs code for: ' + uri.path);
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                this.extension.logger.appendLineToMessages('Cannot read file (' + remoteName + '): ' + ex.message);
                vscode.window.showErrorMessage(ex.message);
                reject(ex);
            }
        });
                ...
                try {
                    if (err) {
                        if (err.code === ErrorCodes.FILE_NOT_FOUND) {
                            ...
                            this.releaseConnection(remoteName, connectionProvider);
                            this.removeWatchLock(lock);
                            this.extension.logger.appendErrorToMessages('writeFile', 'Failed to lstat remote file:' + uri.path, err);
                            reject(err);
                            return;
                        }
                    } else {
                        ...
                        if(!options.overwrite) {
                            ...
                            this.extension.logger.appendErrorToMessages('writeFile', 'Failed write file, file exists and overwrite is false:' + uri.path, err);
                            return;
                        }
                        ...
                        if(stats.isSymbolicLink()) {
                            ...
                            this.extension.logger.appendErrorToMessages('writeFile', 'Failed write file, remote file is a symbolic link:' + uri.path, err);
                            return;
                        }
                        ...
                        if(!stats.isFile()) {
                            ...
                            this.extension.logger.appendErrorToMessages('writeFile', 'Failed write file, remote file is a directory:' + uri.path, err);
                            return;
                        }
                    }
                    ...
                    if (statLocal !== undefined && statLocal.type !== vscode.FileType.File) {
                        this.releaseConnection(remoteName, connectionProvider);
                        this.removeWatchLock(lock);
                        this.extension.logger.appendLineToMessages('[write-file] Local file exists but is not a File: ' + localPath.fsPath);
                        reject(Error('Local file expected to be a file, but it was a directory, file location: ' + localPath.fsPath));
                        this.extension.logger.appendErrorToMessages('writeFile', 'Local file is expected to be a file but a directory was found:' + localPath.path, err);
                        return;
                    }
                    ...
                    try {
                        ...
                        if (parentDirectoryStat === undefined) {
                            ...
                            this.extension.logger.appendLineToMessages('[write-file] Created local dir ' + parentDirectory.fsPath);
                        }
                    } catch(ex: any) {
                        this.releaseConnection(remoteName, connectionProvider);
                        this.extension.logger.appendErrorToMessages('writeFile', 'Failed to create local dir ' + parentDirectory.fsPath + ': ', ex);
                        reject(ex);
                        return;
                    } finally {
                        this.removeWatchLock(lock);
                    ...
                    this.extension.logger.appendLineToMessages('[write-file] Local file updated with content, uploading file to remote..., targetPath:' + uri.path + ', localFile: ' + localPath.fsPath);
                    ...
                    this.releaseConnection(remoteName, connectionProvider);
                    this.removeWatchLock(lock);
                    this.extension.logger.appendErrorToMessages('writeFIle', 'Something went wrong for write operation: ' + uri.path, ex);
                    reject(ex);
                }
            });
        });
    ...
    async createDirectory(uri: vscode.Uri): Promise<void> {
        ...
        await vscode.window.withProgress({
            ...
            return new Promise<void>(async (resolve, reject) => {
                ...
                this.extension.logger.appendLineToMessages('[create dir] ' + uri.path);
                connection.mkdir(uri.path, async (err) => {
                    if (err) {
                        this.releaseConnection(remoteName, connectionProvider);
                        this.extension.logger.appendErrorToMessages('writeFIle', 'Failed to create remote directory: ' + uri.path, err);
                        return reject(err);
                    }
                    this.releaseConnection(remoteName, connectionProvider);
                    ...
                    const lock = this.addWatchLockFromLocalUri(localPath);
                    try {
                        this.extension.logger.appendLineToMessages('[create local dir] ' + localPath.fsPath);
                        await vscode.workspace.fs.createDirectory(localPath);
                    } catch(ex: any) {
                        this.extension.logger.appendErrorToMessages('createDirectory', 'Failed to create local directory: ' + localPath.fsPath, ex);
                    } finally {
                        this.removeWatchLock(lock);
                    }
    ...
    async delete(uri: vscode.Uri, options: { readonly recursive: boolean; }): Promise<void> {
        ...
        this.extension.logger.appendLineToMessages('[delete] Delete request: ' + uri.path + ', recursive: ' + options.recursive);
        ...
        try {
            ...
            if (stat.type === vscode.FileType.Directory) {
                this.extension.logger.appendLineToMessages('[delete] Delete folder: ' + uri.path + ', recursive: ' + recursive);
                ...
                this.extension.logger.appendLineToMessages('[delete] Delete file: ' + uri.path);
            ...
            this.extension.logger.appendErrorToMessages('delete', 'Something went wrong for delete operation: ' + uri.path, ex);
            await this.releaseConnection(remoteName, connectionProvider);
            throw ex;
        }
    }
    ...
    private deleteFile(uri: vscode.Uri, connection: SFTPWrapper, token: vscode.CancellationToken | undefined) {
        ...
        return new Promise<void>((resolve, reject) => {
            ...
            this.extension.logger.appendLineToMessages('Deleting remote file ' + uri.path);
            vscode.window.setStatusBarMessage('Deleting ' + uri.path + '...', 1000);
            connection.unlink(uri.path, async (err) => {
                if (err) {
                    this.extension.logger.appendErrorToMessages('deleteFile', 'Failed to unlink remote file: ' + uri.path, err);
                    reject(err);
                    return;
                }
                ...
                try {
                    ...
                    this.extension.logger.appendErrorToMessages('deleteFile', 'Failed to delete local folder: ' + localPath.fsPath, ex);
                } finally {
                    this.removeWatchLock(lock);
                }
    ...
    private deleteDirectory(uri: vscode.Uri, recursive: boolean, client: SFTPWrapper, token: vscode.CancellationToken | undefined) {
        ...
        return new Promise<void>((resolve, reject) => {
            ...
            if (!recursive) {
                this.extension.logger.appendLineToMessages('Deleting remote folder ' + uri.path);
                vscode.window.setStatusBarMessage('Deleting ' + uri.path + '...', 1000);
                client.rmdir(uri.path, async (err) => {
                    if (err) {
                        this.extension.logger.appendErrorToMessages('deleteDirectory', 'Failed to delete remote directory: ' + uri.path, err);
                        reject(err);
                        return;
                    }
                    ...
                    try {
                        ...
                        this.extension.logger.appendErrorToMessages('deleteDirectory', 'Failed to delete local directory: ' + localPath.fsPath, ex);
                    } finally {
                        this.removeWatchLock(lock);
                    }
            ...
            this.readDirectory(uri).then(
                async (fileEntries) => {
                    try {
                        ...
                        this.extension.logger.appendErrorToMessages('deleteDirectory', 'Failed to perform recursive delete for directory: ' + uri.path, ex);
                        reject(ex);
                    }
                },
                err => {
                ...
                this.extension.logger.appendErrorToMessages('deleteDirectory', 'Failed to read remote directory: ' + uri.path, ex);
                reject(ex);
            });
        });
    }
            ...
            try {
                ...
                if (ex instanceof vscode.FileSystemError || ex.code === ErrorCodes.FILE_NOT_FOUND) {
                    if (ex.code === 'FileNotFound' || ex.code === ErrorCodes.FILE_NOT_FOUND) {
                        ...
                        this.releaseConnection(remoteName, connectionProvider);
                        this.extension.logger.appendErrorToMessages('rename', 'Failed to stat remote oldUri: ' + oldUri.path, ex);
                        reject(ex);
                        return;
                    }
                } else {
            ...
            if (isOldLocal) {
                try {
                    ...
                    try {
                        ...
                        if (ex instanceof vscode.FileSystemError || ex.code === ErrorCodes.FILE_NOT_FOUND) {
                            if (ex.code === 'FileNotFound' || ex.code === ErrorCodes.FILE_NOT_FOUND) {
                                ...
                                this.extension.logger.appendErrorToMessages('rename', 'Failed to stat remote newUri: ' + newUri.path, ex);
                                this.releaseConnection(remoteName, connectionProvider);
                                reject(ex);
                                return;
                            }
                    ...
                    if (targetExistsRemote) {
                        ...
                        this.extension.logger.appendErrorToMessages('rename', 'Failed to make operation, remote file exists: ' + newUri.path, error);
                        reject(error);
                        return;
                    }
                    ...
                    this.extension.logger.appendErrorToMessages('rename', 'Failed to make operation, something went wrong: oldUri = ' + oldUri.path + ', newUri = ' + newUri.path, ex);
                    reject(ex);
                }
                return;
            }
            
            this.extension.logger.appendLineToMessages('[rename] From ' + oldUri.path + ' to ' + newUri.path);
            connection.rename(oldUri.path, newUri.path, async (err) => {
                if (err) {
                    this.extension.logger.appendErrorToMessages('rename', 'Failed to make operation, remote error: oldUri = ' + oldUri.path + ', newUri = ' + newUri.path, err);
                    this.releaseConnection(remoteName, connectionProvider);
                    reject(err);
                    return;
                }
                ...
                try {
                    ...
                    if (oldStats !== undefined) {
                        this.extension.logger.appendLineToMessages('[rename-local] From ' + oldLocalUri.fsPath + ' to ' + newLocalUri.fsPath);
                        await vscode.workspace.fs.rename(oldLocalUri, newLocalUri, { overwrite: true});
                    }
                } catch(ex: any) {
                    this.extension.logger.appendErrorToMessages('rename', 'Failed to make operation, local rename error: oldUri = ' + oldLocalUri.path + ', newUri = ' + newLocalUri.path, ex);
                } finally {
        ...
        return new Promise(async (resolve, reject) => {
            sftp.realpath(uri.path, (err, resolvedPath) => {
                ...
                sftp.lstat(resolvedPath, (err, resolvedStats) => {
                    if (err) {
                        this.extension.logger.appendErrorToMessages('followSymbolicLinkAndGetStats', 'Failed to follow symbolic link, lstat error: ' + uri.path, err);
                        reject(err);
                        return;
                    }
    ...
    async followSymbolicLinkAndGetRealPath(uri: vscode.Uri, connectionSFTP: SFTPWrapper | undefined = undefined): Promise<vscode.Uri> {
        ...
        try {
            ...
            this.extension.logger.appendErrorToMessages('followSymbolicLinkAndGetRealPath', 'Failed to follow symbolic link: ' + uri.path, ex);
            await this.releaseConnection(remoteName, connectionProvider);
            throw ex;
        }
    }
    ...
    asyncFollowSymbolicLinkAndGetRealPath(sftp: SFTPWrapper, uri: vscode.Uri): Promise<vscode.Uri> {
        return new Promise(async (resolve, reject) => {
            sftp.realpath(uri.path, (err, resolvedPath) => {
                if (err) {
                    this.extension.logger.appendErrorToMessages('asyncFollowSymbolicLinkAndGetRealPath', 'Failed to follow symbolic link, realpath failed: ' + uri.path, err);
                    reject(err);
                    return;
                }
    ...
    async statLocalFileByUri(uri: vscode.Uri): Promise<vscode.FileStat | undefined> {
        try {
            ...
            this.extension.logger.appendErrorToMessages('statLocalFileByUri', 'Failed to stat local file: ' + uri.path, error);
    ...
    async resolveWhatFileIsNewer(localStats: vscode.FileStat, remoteStats: Stats | vscode.FileStat): Promise<'same' | 'local_newer' | 'remote_newer' | 'same_mtime_different_size'> {
        try {
            ...
            this.extension.logger.appendErrorToMessages('resolveWhatFileIsNewer', 'Failed to compare files: ', error);
    ...
    async openLocalFolderInExplorer(uri: vscode.Uri) {
        ...
        try {
            ...
            this.extension.logger.appendErrorToMessages('openLocalFolderInExplorer', 'Failed to open file using system explorer: ' + uri.fsPath, error);
    ...
    async syncRemoteFolderWithLocal(uri: vscode.Uri, progress: vscode.Progress<{ message?: string; increment?: number; }>, token: vscode.CancellationToken): Promise<void> {
        ...
        return new Promise(async (resolve, reject) => {
            try {
                this.extension.logger.appendLineToMessages('[sync-files] ' + uri.path);
                ...
                connection.lstat(uri.path, async(error, remoteStat) => {
                    try {
                        ...
                        if (error) {
                            this.extension.logger.appendErrorToMessages('syncRemoteFolderWithLocal', 'Failed to lstat remote file: ' + uri.fsPath, error);
                            reject(error);
                            return;
                        }
                        
                        if (!remoteStat.isDirectory()) {
                            this.extension.logger.appendErrorToMessages('syncRemoteFolderWithLocal', 'Remote file is not a directory: ' + uri.fsPath, Error('Remote file is not a directory.'));
                                ...
                                new Promise(async (resolve, reject) => {
                                    try {
                                        ...
                                        if (ex.code === ErrorCodes.FILE_NOT_FOUND) {
                                            this.extension.logger.appendErrorToMessages('syncRemoteFolderWithLocal', 'File not found, but skipped because this can happen if the file was removed after the calculation of the operation list: ' + uri.fsPath + ', operation: ' + operation, ex);
                                            resolve();
                                            return;
                                        }
                                        
                                        this.extension.logger.appendErrorToMessages('syncRemoteFolderWithLocal', 'Something went wrong: ' + uri.fsPath + ', operation: ' + operation, ex);
                                        reject(ex);
                                    }
                                })
                            );
                        ...
                        this.extension.logger.appendErrorToMessages('syncRemoteFolderWithLocal', 'Something went wrong: ' + uri.fsPath, ex);
                        reject(ex);
                    }
                });
            } catch(ex: any) {
    ...
    private async createOperationListForBothSync(
        ...
        return new Promise(async (resolve, reject) => {
            try {
                this.extension.logger.appendLineToMessages('[operations-sync] To analyze remote: ' + remoteFiles.length);
                            ...
                            try {
                                if (remoteType === vscode.FileType.File) {
                                    ...
                                    if (localStats === undefined) {
                                        ...
                                        this.extension.logger.appendLineToMessages('[operations-sync] Missing local file, resolution DOWNLOAD: ' + remoteUri.path);
                                        resolve();
                                        return;
                                    }
                                    ...
                                    if (compare === 'local_newer') {
                                        ...
                                        this.extension.logger.appendLineToMessages('[operations-sync] Local file is newer, resolution UPLOAD: ' + remoteUri.path);
                                    } else if(compare === 'remote_newer') {
                                        // Download required
                                        res.push([remoteUri, 'DOWNLOAD_REMOTE']);
                                        this.extension.logger.appendLineToMessages('[operations-sync] Remote file is newer, resolution DOWNLOAD: ' + remoteUri.path);
                                    } else {
                                        res.push([remoteUri, 'NOTHING']);
                                        this.extension.logger.appendLineToMessages('[operations-sync] Both files are synced, resolution NOTHING: ' + remoteUri.path);
                                    }
                                }
                                
                                resolve();
                            } catch(ex: any) {
                                this.extension.logger.appendErrorToMessages('createOperationListForBothSync', 'Something went wrong while comparing files: ' + remoteUri.path, ex);
                                reject(ex);
                            }
                        })
                    );
                ...
                this.extension.logger.appendLineToMessages('[operations-sync] To analyze local: ' + localFiles.length);
                ...
                for (const localEntry of localFiles) {
                    ...
                    if (localType !== vscode.FileType.File) {
                        this.extension.logger.appendLineToMessages('[operations-sync] Local file is not file, resolution DISCARD: ' + localUri.path);
                        continue;
                    }
                    ...
                    if (found === true) {
                        this.extension.logger.appendLineToMessages('[operations-sync] Local file already processed in remote list, resolution DISCARD: ' + localUri.path);
                        continue;
                    }
                    ...
                    this.extension.logger.appendLineToMessages('[operations-sync] Remote file is not present, resolution UPLOAD: ' + remoteUri.path);
                    res.push([remoteUri, 'UPLOAD_LOCAL']);
                }
                ...
                this.extension.logger.appendErrorToMessages('createOperationListForBothSync', 'Something went wrong: ', ex);
                reject(ex);
            }
        });
    }
    ...
    async uploadRemoteFolderFromLocal(uri: vscode.Uri, progress: vscode.Progress<{ message?: string; increment?: number; }>, token: vscode.CancellationToken): Promise<void> {
        ...
        return new Promise(async (resolve, reject) => {
            this.extension.logger.appendLineToMessages('[read-file] ' + uri.path);
            ...
            connection.lstat(uri.path, async(error, remoteStat) => {
                ...
                if (error) {
                    this.extension.logger.appendErrorToMessages('uploadRemoteFolderFromLocal', 'Failed to lstat remote: ' + uri.path, error);
                    reject(error);
                    return;
                }
                
                if (!remoteStat.isDirectory()) {
                    this.extension.logger.appendErrorToMessages('uploadRemoteFolderFromLocal', 'Remote file is not a directory: ' + uri.path, Error());
                    reject(Error('Remote file is not a directory.'));
                    return;
                }
                ...
                try {
                    ...
                    this.extension.logger.appendErrorToMessages('uploadRemoteFolderFromLocal', 'Something went wrong while doing updateAll operation: ' + uri.path, ex);
                    reject(ex);
                }
            });
        });
    ...
    async downloadRemoteFolderToLocal(uri: vscode.Uri, progress: vscode.Progress<{ message?: string; increment?: number; }>, token: vscode.CancellationToken): Promise<void> {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            connection.lstat(uri.path, async(error, remoteStat) => {
                ...
                if (error) {
                    this.extension.logger.appendErrorToMessages('uploadRemoteFolderFromLocal', 'Failed to lstat remote file: ' + uri.path, error);
                    reject(error);
                    return;
                }
                ...
                try {
                    ...
                    this.extension.logger.appendErrorToMessages('uploadRemoteFolderFromLocal', 'Something went wrong while downloading all files: ' + uri.path, ex);
                    reject(ex);
                }
            });
        });
    ...
    private async listLocalFolder(uri: vscode.Uri,  token: vscode.CancellationToken): Promise<[vscode.Uri, vscode.FileType][]> {
        return new Promise(async (resolve, reject) => {
            ...
            this.extension.logger.appendLineToMessages('[upload-from-local] Listing dir: ' + uri.path);
            this.extension.vscodeStatusBarItem.text = '$(search) Listing ' + uri.path;
            ...
            try {
                    ...
                    promisesEntries.push(
                        new Promise<[vscode.Uri, vscode.FileType][]>(async (resolve, reject) => {
                            ...
                            try {
                                ...
                                this.extension.logger.appendErrorToMessages('listLocalFolder', 'Failed to list local folder: ' + uri.path, ex);
                                reject(ex);
                            }
                        })
                    );
                ...
                if (ex instanceof vscode.FileSystemError && ex.code === 'FileNotFound') {
                    ...
                    this.extension.logger.appendErrorToMessages('listLocalFolder', 'Failed to list local folder, something went wrong: ' + uri.path, ex);
                    reject(ex);
                }
            }
        });
    ...
    private async listRemoteFolder(uri: vscode.Uri,  token: vscode.CancellationToken): Promise<[vscode.Uri, vscode.FileType][]> {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            // Uri for local folder
            this.extension.logger.appendLineToMessages('[download-from-remote] Listing dir: ' + uri.path);
            this.extension.vscodeStatusBarItem.text = '$(search) Listing ' + uri.path;
            connection.readdir(uri.path, async (err, entries) => {
                this.releaseConnection(remoteName, connectionProvider);
                
                if (err) {
                    this.extension.logger.appendErrorToMessages('listRemoteFolder', 'Failed to readdir: ' + uri.path, err);
                    reject(err);
                    return;
                }
                        ...
                        promisesEntries.push(
                            new Promise<[vscode.Uri, vscode.FileType][]>(async (resolve, reject) => {
                                ...
                                try {
                                    ...
                                    this.extension.logger.appendErrorToMessages('listRemoteFolder', 'Failed to list remote directory, something went wrong: ' + uri.path, ex);
                                    reject(ex);
                                }
                            })
                        );
                        ...
                        new Promise<void>(async (resolve, reject) => {
                            try {
                                ...
                                if (fileType === vscode.FileType.File) {
                                    this.extension.logger.appendLineToMessages('[upload-from-local] Uploading file: ' + localPath.path);
                                    // In case of file, upload it
                                    await this.uploadLocalFileToRemoteIfNeeded(remoteName, localPath, 'heavy', false, token);
                                    const remotePath = this.getRemoteFileUri(remoteName, localPath);
                                    this.extension.sftpFileDecoration.setUpToDateFileDecoration(remotePath);
                                    this.uploadedCount++;
                                    progress.report({
                                ...
                                if (ex.code === ErrorCodes.FILE_NOT_FOUND) {
                                    this.extension.logger.appendErrorToMessages('uploadAllFiles', 'File not found, but skipped because this can happen if the file was removed after the calculation of the operation list: ' + localPath.fsPath, ex);
                                    resolve();
                                    return;
                                }
                                
                                this.extension.logger.appendErrorToMessages('uploadAllFiles', 'Failed to upload from local path: ' + localPath.path, ex);
                                reject(ex);
                            }
                        })
                    );
                    ...
                    promisesEntries.push(
                        new Promise<void>(async (resolve, reject) => {
                            try {
                                ...
                                if (fileType === vscode.FileType.File) {
                                    this.extension.logger.appendLineToMessages('[download-from-remote] Downloading file: ' + remotePath.path);
                                ...
                                if (ex.code === ErrorCodes.FILE_NOT_FOUND) {
                                    this.extension.logger.appendErrorToMessages('downloadAllFiles', 'File not found, but skipped because this can happen if the file was removed after the calculation of the operation list: ' + remotePath.fsPath + ', operation: ', ex);
                                    resolve();
                                    return;
                                }
                                
                                this.extension.logger.appendErrorToMessages('downloadAllFiles', 'Failed to download from remote path: ' + remotePath.path, ex);
                                reject(ex);
                            }
                        })
                    );
            ...
            try {
                ...
                try {
                    ...
                    if (ex instanceof vscode.FileSystemError) {
                        if (ex.code !== 'FileNotFound') {
                            this.extension.logger.appendErrorToMessages('uploadLocalFileToRemoteIfNeeded', 'Something went wrong while getting stat of remote file: ' + localUri.path, ex);
                            reject(ex);
                            return;
                        }
                    } else {
                ...
                if (remoteStat !== undefined) {
                    ...
                    if (compare === 'remote_newer' && !forceRemoteOverwrite) {
                        ...
                        this.extension.sftpFileDecoration.setRemoteDownloadFileDecoration(remoteUri);
                        resolve();
                        return;
                    }
                }
                ...
                try {
                    ...
                    // Upload file...
                    // upload file to remote
                    this.extension.vscodeStatusBarItem.text = '$(cloud-upload) ' + remoteUri.path;
                    const sizeKB = this.extension.configuration.getBehaviorNotificationUploadKB();
                                ...
                                connection.fastPut(
                                    ...
                                    remoteUri.path,
                                    {
                                        ...
                                        step(total, nb, fileSize) {
                                            ...
                                            that.extension.logger.appendLineToMessages('[upload-file ' + filename + '] Progress "' + total + '" of "' + fileSize + '" transferred.');
                    ...
                    this.extension.logger.appendLineToMessages('[upload-remote-file] Write completed, mtime must be updated for: ' + localUri.path);
                    connection.lstat(remoteUri.path, async (err: any, stats) => {
                        if (err) {
                            ...
                            this.extension.logger.appendErrorToMessages('uploadLocalFileToRemoteIfNeeded', 'Something went wrong while updating mtime of local file: ', err);
                            this.releaseConnection(remoteName, connectionProvider);
                        ...
                        try {
                            ...
                            this.extension.sftpFileDecoration.setUpToDateFileDecoration(remoteUri);
                        } catch(ex: any) {
                            this.releaseConnection(remoteName, connectionProvider);
                            this.removeWatchLock(lock);
                            this.extension.logger.appendErrorToMessages('uploadLocalFileToRemoteIfNeeded', 'Something went wrong: ', ex);
                            reject(ex);
                        }
                    });
                } finally {
                ...
                try {
                    ...
                    if (localFileStat !== undefined) {
                        ...
                        if (comparisonResult === 'same' && !forceLocalDownload) {
                            this.extension.logger.appendLineToMessages('[read-file] ' + uri.path + ' -> Local file exists and is the same as remote, using local file., rmtime: ' + (remoteStat.mtime * 1000) + ', ltime: ' + localFileStat.mtime);
                            ...
                            this.extension.logger.appendLineToMessages('[read-file] ' + uri.path + ' -> Local file exists and is newer than remote, rmtime: ' + (remoteStat.mtime * 1000) + ', ltime: ' + localFileStat.mtime);
                            ...
                            this.extension.sftpFileDecoration.setRemoteDownloadFileDecoration(uri);
                            this.extension.logger.appendLineToMessages('[read-file] ' + uri.path + ' -> Remote is newer than local file, download needed, rmtime: ' + (remoteStat.mtime * 1000) + ', ltime: ' + localFileStat.mtime);
                        } else if(comparisonResult === 'same_mtime_different_size') {
                            // TODO: how handle this conflict?
                            this.extension.sftpFileDecoration.setUnknownStateFileDecoration(uri);
                            this.extension.logger.appendLineToMessages('[read-file] ' + uri.path + ' -> Remote and local have same mtime, but different sizes, download needed., rmtime: ' + (remoteStat.mtime * 1000) + ', ltime: ' + localFileStat.mtime);
                        }
                    }
                    ...
                    this.extension.vscodeStatusBarItem.text = '$(cloud-download) ' + uri.path;
                    const sizeKB = this.extension.configuration.getBehaviorNotificationDownloadKB();
                    ...
                    await new Promise<void>((resolve) => {
                        connection.lstat(uri.path, (err, stats) => {
                            if (err) {
                                // This error is not really important, so instead of fail the entire process only print this in the logs...
                                this.extension.logger.appendErrorToMessages('downloadRemoteFileToLocalIfNeeded', 'Something went wrong while reading lstat of remote: ' + uri.path, err);
                                resolve();
                                return err;
                            }
                            ...
                            fs.utimes(localPath.fsPath, stats.atime, stats.mtime, (err) => {
                                if (err) {
                                    // This error is not really important, so instead of fail the entire process only print this in the logs...
                                    this.extension.logger.appendErrorToMessages('downloadRemoteFileToLocalIfNeeded', 'Something went wrong while updating mtime of local file: ' + localPath.fsPath, err);
                                }
                                resolve();
                            });
                        });
                    ...
                    this.extension.sftpFileDecoration.setUpToDateFileDecoration(uri);
                    this.extension.logger.appendLineToMessages('[download-file] Completed for: ' + filename);
                    await this.releaseConnection(remoteName, connectionProvider);
                    resolve(res);
                } catch(ex: any) {
                    await this.releaseConnection(remoteName, connectionProvider);
            ...
            connection.fastGet(
                ...
                calculatedLocalFile.fsPath, 
                {
                    ...
                    step(total, nb, fileSize) {
                        ...
                        that.extension.logger.appendLineToMessages('[download-file ' + filename + '] Progress "' + total + '" of "' + fileSize + '" transferred.');
                        progress?.report({ increment: (nb / fileSize) * 100 }); 
                    }
                }, 
                async (err) => {
    ...
    async getConnection(remoteName: string, type: PoolType) {
        this.extension.logger.appendLineToMessages('[connection] Trying to acquire "' + type + '" connection.');
    ...
    async releaseConnection(remoteName: string, connection: ConnectionProvider | undefined) {
        try {
            ...
            this.extension.logger.appendLineToMessages('[connection] Releasing "' + connection.type + '" connection.');
            
            (await this.extension.connectionManager.get(remoteName)?.getPool(connection.type))?.release(connection);
            
            this.extension.logger.appendLineToMessages('[connection] Connection "' + connection.type + '" released.');
        } catch(ex: any) {
            this.extension.logger.appendErrorToMessages('releaseConnection', 'Error releasing connection:', ex);
            // Do nothing...
        }
    }
    ...
    private _fireSoon(...events: vscode.FileChangeEvent[]): void {
        ...
        this._fireSoonHandle = setTimeout(() => {
            console.log('Fire soon! Events: ' + this._bufferedEvents.length);
    ...
    private addWatchLockFromLocalUri(localPath: vscode.Uri): WatchLock {
        this.extension.logger.appendLineToMessages('[watcher] Adding locking for: ' + localPath.fsPath);
    ...
    private removeWatchLock(lock: WatchLock) {
        ...
        this.extension.logger.appendLineToMessages('[watcher] Remove locking for: ' + lock.lockedEntry.fsPath);
    }
    ...
    private pruneExpiredLocks() {
        ...
        if (expiredEntries.length !== 0) {
            this.extension.logger.appendLineToMessages('[warn] [watcher-cleanup] ' + expiredEntries.length + ' locks expired, that should not happen.');
    ...
    async refreshDirectoryContent(uri: vscode.Uri) {
        // Get existing cache
        const cacheEntry = this.cachedRemoteDirectoryFiles.get(uri.toString());
        
        const dir = await this.$readDirectory(uri, false);
        console.log('Refreshed directory: ' + uri.path + ', files: ' + dir.length);
        
        // First remove files from vscode.
        if (cacheEntry !== undefined) {
            for (const entry of cacheEntry.files) {
                this._fireSoon({ type: vscode.FileChangeType.Deleted, uri: uri.with({ path: upath.join(uri.path, entry[0]) }) });
            }
        }
        
        // Then add new files to vscode.
        dir.forEach((f) => {
            this._fireSoon({ type: vscode.FileChangeType.Created, uri: uri.with({ path: upath.join(uri.path, f[0]) }) });
        });
    }
    
    async closeVsCodeTabByFileUri(uri: vscode.Uri) {
...
export class SFTPFileProviderData {
    ...
    cleanCachedDirectories() {
        ...
        for (const entry of this.cachedStatDirectories) {
            ...
            if (Date.now() - cache.timestamp >= (this.extension.configuration.getCacheMetadataTimeToKeep() * 1000)) {
                this.extension.logger.appendLineToMessages('[cache] Removed entry for directory: ' + key);
                toRemove.push(key);
            }
        }
...
export interface DirectoryCachedFiles {
