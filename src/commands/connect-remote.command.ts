...
export class ConnectRemoteCommand extends BaseCommand {
  ...
  async callback() {
    ...
    if (this.extension.vscodeRemoteConnectQuickPick === undefined) {
      this.extension.vscodeRemoteConnectQuickPick =
        vscode.window.createQuickPick<QuickPickItemWithValue>();

      const didAcceptDisposable = this.extension.vscodeRemoteConnectQuickPick.onDidAccept(
        async () => {
          if (!this.extension.vscodeRemoteConnectQuickPick) {
            return;
          }
          const selectedItem = this.extension.vscodeRemoteConnectQuickPick
      ...
      const didTriggerItemButtonDisposable =
        this.extension.vscodeRemoteConnectQuickPick.onDidTriggerItemButton(async (event) => {
