...
export class ShowSystemExplorerCommand extends BaseCommand {
  async callback(uri: vscode.Uri) {
    try {
      this.extension.logger.appendLineToMessages("Show in system explorer for file, scheme=" + uri.scheme + ", authority=" + uri.authority + ", path=" + uri.path);
      ...
      if (statFile.type === vscode.FileType.Directory) {
        ...
        if (localFileStats === undefined) {
          ...
          this.extension.logger.appendLineToMessages('Making folder... ' + calculatedLocalFile.fsPath);
          await vscode.workspace.fs.createDirectory(calculatedLocalFile);
        }
        
        // open...
        this.extension.logger.appendLineToMessages('Opening folder... ' + calculatedLocalFile.fsPath);
