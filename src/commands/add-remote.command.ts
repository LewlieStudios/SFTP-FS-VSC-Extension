...
export class AddRemoteCommand extends BaseCommand {
  async callback() {
    const data = await AddRemoteCommand.showFormModifyRemoteConfiguration(this.extension, undefined);
    if (!data) {
      return;
    }
    
    // Save configuration...
    this.extension.configuration.saveRemoteConfiguration(
      data.name,
      data.host,
      parseInt(data.port),
      data.username,
    ...
    .catch((ex) => {
      ...
      this.extension.logger.appendErrorToMessages('sftpfs.addRemote', 'Unable to save remote configuration.', ex);
    });
  }
  
  static async showFormModifyRemoteConfiguration(extension: SFTPExtension, existingConfigurationName: string | undefined) {
    const existingConfiguration: RemoteConfiguration | undefined = existingConfigurationName ? extension.configuration.getRemoteConfiguration(existingConfigurationName) : undefined;
